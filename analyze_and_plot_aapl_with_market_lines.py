...
print("\n" + "="*50)
print("PLOTTING AAPL DATA WITH MARKET OPEN/CLOSE LINES")
print("="*50)

# Plot AAPL data using index instead of date with market lines
if len(aapl_data) > 0:
    ...
    # Find market open and close indices
    # Market open: 14:30 UTC (14.5 in decimal)
    # Market close: 21:00 UTC (21.0 in decimal)
    market_open_time = 14.5  # 14:30 UTC
    market_close_time = 21.0  # 21:00 UTC
    ...
    for date, day_data in aapl_data_sorted.groupby(aapl_data_sorted['date'].dt.date):
        # Find market open (closest to 14:30 UTC)
        ...
        # Find market close (closest to 21:00 UTC)
        close_mask = day_data['time_decimal'] >= market_close_time
        if close_mask.any():
            market_close_idx = day_data[close_mask].index[0]
            market_close_indices.append(market_close_idx)
        else:
            # If no 21:00 data, use the last record of the day
    ...
    plt.title('AAPL Price Data with Market Open/Close Lines')
    ...
    plt.title('AAPL Trading Volume with Market Open/Close Lines')
    plt.xlabel('Time Step Index')
    plt.ylabel('Volume')
    plt.grid(True, alpha=0.3)
    ...
    plt.title('AAPL Daily Price Range with Market Open/Close Lines')
    plt.xlabel('Time Step Index')
    ...
    plt.title('AAPL Close Price with Moving Averages and Market Lines')
    plt.xlabel('Time Step Index')
    plt.ylabel('Price ($)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.figtext(0.02, 0.02, 'Green dashed lines: Market Open (14:30 UTC)\nRed dashed lines: Market Close (21:00 UTC)', 
                fontsize=10, bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))
    
    plt.tight_layout()
    plt.savefig('aapl_analysis_plot_with_market_lines.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("✓ AAPL plots with market lines created and saved as 'aapl_analysis_plot_with_market_lines.png'")
    ...
    print(f"\nMARKET HOURS ANALYSIS:")
    print(f"Market open time: 14:30 UTC (9:30 AM EST)")
    print(f"Market close time: 21:00 UTC (4:00 PM EST)")
...
print(f"- Added vertical lines for market open (14:30 UTC) and close (21:00 UTC)")
