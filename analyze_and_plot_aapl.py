...
if len(aapl_data) > 0:
    ...
    plt.subplot(2, 2, 1)
    plt.plot(aapl_data_sorted['date'], aapl_data_sorted['close'], label='Close', linewidth=2)
    plt.plot(aapl_data_sorted['date'], aapl_data_sorted['high'], label='High', alpha=0.7)
    plt.plot(aapl_data_sorted['date'], aapl_data_sorted['low'], label='Low', alpha=0.7)
    plt.plot(aapl_data_sorted['date'], aapl_data_sorted['open'], label='Open', alpha=0.7)
    plt.title('AAPL Price Data Over Time')
    plt.xlabel('Date')
    plt.ylabel('Price ($)')
    plt.legend()
    ...
    # Subplot 2: Volume
    plt.subplot(2, 2, 2)
    plt.plot(aapl_data_sorted['date'], aapl_data_sorted['volume'], color='orange', linewidth=2)
    ...
    plt.plot(aapl_data_sorted['date'], price_range, color='red', linewidth=2)
    ...
    plt.subplot(2, 2, 4)
    plt.plot(aapl_data_sorted['date'], aapl_data_sorted['close'], label='Close Price', linewidth=2)
    
    # Add moving averages if we have enough data
    if len(aapl_data_sorted) > 7:
        ma_7 = aapl_data_sorted['close'].rolling(window=7).mean()
        plt.plot(aapl_data_sorted['date'], ma_7, label='7-period MA', alpha=0.7)
    
    if len(aapl_data_sorted) > 14:
        ma_14 = aapl_data_sorted['close'].rolling(window=14).mean()
        plt.plot(aapl_data_sorted['date'], ma_14, label='14-period MA', alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig('aapl_analysis_plot.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("✓ AAPL plots created and saved as 'aapl_analysis_plot.png'")
